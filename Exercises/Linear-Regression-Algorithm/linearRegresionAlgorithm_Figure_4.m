...
% Number of points
nData = 10:5:500; % From [10,500]


for j=1:1:length(nData)
    
    % Generate data
    [xdata,ydata] = generateData(nData(j));
    
    % Loop for every division
    for i=1:1:k_div
        
        MSE_div = zeros(k_div,1);
        
        % Split train and test data
        [Xtrain,Ytrain,Xtest,Ytest] = splitDataTrainAndTest(xdata,ydata,nData(j), i, k_div);
...
xlabel('MSE');
ylabel('Iterations');
title('\textbf{Mean Square Error using k-fold cross validation as a function of nData}');
...
% print(plot_pdf, 'MSE_kfold_nData.pdf', '-dpdf', '-r0')
% 
% % Save png
% print(gcf,'MSE_kfold_nData.png','-dpng','-r600');

plot_pdf2 = figure(2);
errorbar(nData,mean_MSE(1,:),std_MSE(1,:));
grid on;
grid minor;
box on;
xlabel('nData');
...
%     'PaperSize',[pos(3), pos(4)]);
% print(plot_pdf2, 'MSE_kfold_nData_error.pdf', '-dpdf', '-r0')
% 
% % Save png
% print(gcf,'MSE_kfold_nData_error.png','-dpng','-r600');
